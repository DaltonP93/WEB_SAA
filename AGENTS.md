...
✅ **Tests automatizados**: `pnpm test` (vitest) — **1497 pruebas en 82 archivos**
tras la segunda ronda correctiva del PR #23 (bandeja de newsletter: filas de la
respuesta anterior no accionables y offset que vuelve a la última página válida;
semántica explícita de Publicar/Despublicar/Programar con la decisión de "fecha
futura" en el backend, zona Asunción; restauración fiel del instante de
`publish_at`; consentimiento con fuente única y baja con `unsubscribed_at`), sobre
la primera ronda correctiva (**1486 en 80**: historial recuperable con guardado
atómico `/content`, restauración con confirmación, publicación programada desde
borrador, papelera consistente y newsletter con consentimiento + baja por token).
Las que necesitan base real se activan con
