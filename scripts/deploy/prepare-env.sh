...
if [ -f "$ENV_FILE" ]; then
  EXTRA_KEYS="$(grep -E '^[A-Z_][A-Z0-9_]*=' "$ENV_FILE" \
    | grep -vE '^(PORT|NODE_ENV|DB_HOST|DB_PORT|DB_USER|DB_PASS|DB_NAME|JWT_SECRET|JWT_EXPIRES_IN|UPLOAD_DIR|UPLOAD_STAGING_DIR|MAX_UPLOAD_MB|CORS_ORIGINS|PUBLIC_BASE_URL|PUBLIC_SITE_URL|SEED_ADMIN_EMAIL|SEED_ADMIN_NAME|SEED_ADMIN_PASSWORD)=' \
    || true)"
fi
...
# Hermano de uploads, nunca adentro; mismo filesystem para rename atómico.
UPLOAD_STAGING_DIR=${APP_DIR}/api/.uploads-staging
MAX_UPLOAD_MB=10
