  ...
  return (
    <section className="container-x section-y-md">
      ...
      {isLoading ? (
        ...
        <div className={`grid grid-cols-1 ${cols} gap-5`}>
          {(data ?? []).map((n: any) => (
            <Link to={`/noticias/${n.slug}`} key={n.id} className="group bg-white rounded shadow overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
              {n.cover_url && <img src={n.cover_url} alt={n.title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />}
              <div className="p-4">
      ...
      {(!isLoading && (!data || data.length === 0)) && <p className="text-gray-500">Próximamente publicaremos novedades.</p>}
    </section>
  );
}
