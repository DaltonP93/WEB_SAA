  ...
  return (
    <section className="container-x section-y-md">
      ...
      <div className={`grid grid-cols-1 ${cols} gap-6`}>
        {items.map((it, i) => (
          <a key={i} href={it.href ?? "#"} className="group block bg-white rounded shadow hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden">
            {it.imageUrl && <img src={it.imageUrl} alt={it.title} className="w-full h-44 object-cover group-hover:scale-105 transition-transform duration-300" />}
            <div className="p-4">
