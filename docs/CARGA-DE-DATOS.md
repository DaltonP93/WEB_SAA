...
2. **Registrar la confirmación**, que es la constancia de que el sanatorio
   afirma ese alcance. Se hace desde **Panel → Datos pendientes**, en la
   tarjeta *Alcance de Biopsias*: el botón **Registrar confirmación** pide qué
   se confirma —qué estudios, con qué requisitos, en qué plazos— y lo guarda
   con tu nombre y la fecha. Sólo aparece para usuarios con rol **superadmin**;
   un editor ve el estado y a quién pedírselo.
...
Lo que se escriba en *"Qué se confirma"* queda como constancia y se muestra en
la misma pantalla. Escribilo con suficiente detalle para que otra persona pueda
revisarlo dentro de seis meses: "está todo bien" no dice qué está bien.

Si el alcance cambia —cambian los plazos, se deja de hacer un estudio— se usa
**Retirar confirmación** en esa misma tarjeta y el ítem vuelve a revisión.
